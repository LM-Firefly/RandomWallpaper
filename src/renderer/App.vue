        ...
        <el-card>
          <el-collapse v-model="activeCollapse" accordion>
            ...
            <el-collapse-item title="Unsplash" name="unsplash">
               ...
               <el-button type="primary" @click="saveUnsplash" style="margin-top:8px">Save</el-button>
               <el-button @click="testUnsplashKey" style="margin-top:8px">Test Key</el-button>
               <span id="unsplashTestResult" style="margin-left:8px;font-size:smaller"></span>
              <div style="font-size:smaller;margin-top:8px">Note: Only the Access Key (Client ID) is used for public API calls.</div>
            </el-collapse-item>
            ...
            <el-collapse-item title="Reddit" name="reddit">
                ...
                <el-button id="saveReddit" type="primary" @click="saveReddit" style="margin-top:8px">Save</el-button>
            </el-collapse-item>
            ...
            <el-collapse-item title="Wallhaven" name="wallhaven">
              ...
              <el-button id="saveWallhaven" type="primary" @click="saveWallhaven" style="margin-top:8px">Save</el-button>
            </el-collapse-item>
            ...
            <el-collapse-item title="URL Source" name="urlsource">
                ...
                <el-button id="saveUrlSource" type="primary" @click="saveUrlSource" style="margin-top:8px">Save</el-button>
                <div style="font-size:smaller;margin-top:8px">Use full image URLs (http/https). Domain is saved as the source label.</div>
            </el-collapse-item>
            ...
            <el-collapse-item title="Generic JSON" name="genericjson">
                ...
                <div style="margin-top:8px">
                    ...
                    <el-button id="saveGenericJson" type="primary" @click="saveGenericJson" style="margin-top:8px">Save</el-button>
                    <el-button id="fetchGenericJson" @click="fetchGenericJson" style="margin-top:8px">Fetch Generic JSON</el-button>
                </div>
                <div style="font-size:smaller;margin-top:8px">Request a JSON endpoint and extract an image URL using a JSONPath-like path. Prefix is applied to non-http URLs.</div>
            </el-collapse-item>
            ...
            <el-collapse-item title="Auto-change Wallpaper" name="scheduler">
                ...
                <div style="margin-top:8px; display:flex; flex-direction:column; gap:6px">
                  <el-checkbox v-model="autoStartEnabled">Start application on system startup</el-checkbox>
                  <el-checkbox v-model="autoStartSilent">Silent start (start minimized to system tray)</el-checkbox>
                </div>
                <div style="margin-top:8px">
                  <el-button id="schedulerStart" type="primary" @click="schedulerStart" style="margin-top:8px">Start</el-button>
                  <el-button id="schedulerStop" @click="schedulerStop" style="margin-top:8px">Stop</el-button>
                </div>
            </el-collapse-item>
            <el-collapse-item title="Appearance" name="appearance">
              <div style="margin-top:8px; display:flex;align-items:center;gap:12px;flex-wrap:wrap">
                <div style="display:flex;align-items:center;gap:8px">
                  Theme:
                  <el-radio-group v-model="themeManual" size="small">
                    <el-radio label="light">Light</el-radio>
                    <el-radio label="dark">Dark</el-radio>
                  </el-radio-group>
                </div>
                <div style="margin-left:auto"><el-checkbox v-model="followSystem">Follow system theme</el-checkbox></div>
              </div>
            </el-collapse-item>
          </el-collapse>
        </el-card>
...
export default {
  ...
  setup() {
    ...
    const autoStartSilent = ref(false)
    const themeManual = ref<'light' | 'dark'>('light')
    ...
    async function loadSettings() {
      try {
        ...
        try {
          const saved = await window.rw?.themeGet()
          if (saved === 'system') {
            followSystem.value = true
            const sys = await window.rw?.systemThemeNow()
            themeManual.value = sys === 'dark' ? 'dark' : 'light'
            applyTheme(themeManual.value)
          } else if (saved === 'dark' || saved === 'light') {
            followSystem.value = false
            themeManual.value = saved
            applyTheme(saved)
          } else {
            followSystem.value = false
            themeManual.value = 'light'
            applyTheme('light')
          }
        } catch (e) { /* ignore */ }
        // load persisted auto-start flag (fallback to settings store)
        try {
          const as = await window.rw?.autoStartGet()
          if (typeof as === 'boolean') autoStartEnabled.value = as
          else autoStartEnabled.value = (await window.rw?.settingsGet('autoStart', 'enabled')) === 'true'
          // load silent start flag
          try {
            const sSilent = await window.rw?.autoStartSilentGet()
            if (typeof sSilent === 'boolean') autoStartSilent.value = sSilent
            else autoStartSilent.value = (await window.rw?.settingsGet('autoStart', 'silent')) === 'true'
          } catch (_) { autoStartSilent.value = (await window.rw?.settingsGet('autoStart', 'silent')) === 'true' }
        } catch (_) { autoStartEnabled.value = (await window.rw?.settingsGet('autoStart', 'enabled')) === 'true' }
      } catch (e) {
    ...
    function showStatus(msg: string, isError = false) {
      ...
      try { if (statusMessage) statusMessage.textContent = msg } catch (_) {}
      try { ElMessage({ message: String(msg || ''), type: isError ? 'error' : 'success' }) } catch (_) {}
    }
    ...
    function applyTheme(mode: 'light' | 'dark') {
      try {
        ...
        if (!root) return
        // Keep an app-level 'dark' class and also toggle Element Plus theme class
        if (mode === 'dark') {
          root.classList.add('dark')
          root.classList.add('el-theme-dark')
        } else {
    ...
    async function randomFromFolder() {
      ...
      if (!result || result.length === 0) { showStatus('No images', true); return }
    ...
    async function fetchUnsplash() {
      try {
        ...
        if (!res || res.length === 0) { showStatus('No images', true); return }
        ...
        setButton.addEventListener('click', async () => { await window.rw?.downloadAndSet(url); showStatus('Applied'); await loadHistory() })
    ...
    async function saveUnsplash() {
      ...
      showStatus('Saved Unsplash settings')
    }
    ...
    async function saveReddit() {
      ...
      showStatus('Saved Reddit settings')
    }
    ...
    async function saveWallhaven() {
      ...
      showStatus('Saved Wallhaven settings')
    }
    ...
    async function saveUrlSource() {
      ...
      showStatus('Saved URL Source settings')
    }
    ...
    async function saveGenericJson() {
      ...
      showStatus('Saved Generic JSON settings')
    }
    ...
    onMounted(async () => {
      ...
      if (!window.rw) {
        ...
        ;['chooseFolder', 'randomFromFolder', 'fetchUnsplash', 'fetchReddit', 'fetchWallhaven', 'saveUnsplash', 'saveReddit', 'saveWallhaven', 'saveUrlSource', 'saveGenericJson', 'saveGenericJson', 'randomizeNow'].forEach(id => {
          ...
          if (el) el.addEventListener('click', () => showStatus('App preload API is not available. Open DevTools to debug', true))
        })
        return
      }
      ...
      // subscribe to system updates so 'follow system' works
      let sysUnsub: any = null
      let themeUnsub: any = null
      try {
        if (window.rw?.onSystemThemeUpdated) sysUnsub = window.rw.onSystemThemeUpdated((t) => { if (followSystem.value) applyTheme(t === 'dark' ? 'dark' : 'light') })
        if (window.rw?.onThemeUpdated) themeUnsub = window.rw.onThemeUpdated((t) => {
          try {
            if (t === 'system') {
              followSystem.value = true
              window.rw?.systemThemeNow().then(s => { themeManual.value = s === 'dark' ? 'dark' : 'light'; applyTheme(themeManual.value) })
            } else {
              followSystem.value = false
              themeManual.value = (t === 'dark' ? 'dark' : 'light')
              applyTheme(themeManual.value)
            }
          } catch (_) {}
        })
      ...
      onUnmounted(() => {
        try { if (sysUnsub) sysUnsub() } catch (_) {}
        try { if (themeUnsub) themeUnsub() } catch (_) {}
      })
    })
    ...
    // Appearance watch: follow-system and manual theme
    watch(followSystem, async (v) => {
      try {
        if (v) {
          await window.rw?.themeSet('system')
          const sys = await window.rw?.systemThemeNow()
          themeManual.value = sys === 'dark' ? 'dark' : 'light'
          applyTheme(themeManual.value)
        } else {
          await window.rw?.themeSet(themeManual.value)
          applyTheme(themeManual.value)
        }
      } catch (_) {}
    })

    watch(themeManual, async (v) => {
      try {
        if (!followSystem.value) {
          await window.rw?.themeSet(v)
          applyTheme(v)
        }
      } catch (_) {}
    })

    // Persist auto-start and notify main process to update OS startup setting
    watch(autoStartEnabled, async (v) => {
      try { await window.rw?.autoStartSet(!!v) } catch (_) {}
      await window.rw?.settingsSet('autoStart', 'enabled', v ? 'true' : 'false')
    })

    // Persist silent start sub-setting
    watch(autoStartSilent, async (v) => {
      try { await window.rw?.autoStartSilentSet(!!v) } catch (_) {}
      await window.rw?.settingsSet('autoStart', 'silent', v ? 'true' : 'false')
    })
...
<style>
  ...
  html.dark, body.dark, .el-theme-dark, html.el-theme-dark, body.el-theme-dark { background: #0f1720; color: #e6eef6 }
  html.dark .el-card, .el-theme-dark .el-card, html.el-theme-dark .el-card { background: #0b1220; color: #e6eef6 }
  html.dark .el-header, html.dark .el-main, .el-theme-dark .el-header, .el-theme-dark .el-main { background: transparent }
  html.dark h2, .el-theme-dark h2 { color: #e6eef6 }
</style>
