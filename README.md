...
* Adapters: Wallhaven, Reddit, Unsplash, Palace Museum, WallHere, Wallhalla, Generic JSON, Static URL, Local Folder.
...
`%APPDATA%\firefly.randomwallpaper\random-wallpaper.json` under the key
`state`, structured as `{ general, sources, history }`.

The cached wallpaper images live in `%LOCALAPPDATA%\firefly.randomwallpaper\wallpapers\`.
